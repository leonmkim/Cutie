...
def download_models_if_needed():
    ...
    # get a relative path to weights
    ...
    for link, md5 in _links:
        ...
        if not os.path.exists(os.path.join(weights_path, filename)) or hashlib.md5(open(os.path.join(weights_path, filename), 'rb').read()).hexdigest() != md5:
            print(f'Downloading {filename}...')
            r = requests.get(link, stream=True)
            total_size = int(r.headers.get('content-length', 0))
            block_size = 1024
            t = tqdm(total=total_size, unit='iB', unit_scale=True)
            with open(os.path.join(weights_path, filename), 'wb') as f:
